...
/**
 * This servlet class is responsible for adding new group in ontology
 */
@WebServlet("/addRuleGroup")
public class AddRuleGroup extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (FormFieldValidator.isLogin(request.getSession()))
		{
			if(groupName.length()!=0  && groupDescription.length() !=0){
				...
				log.info("Group Name : " + groupName + "\tRule Description : " + groupDescription);
				
				User user = SemTranslator.toUserBeans(userName, "", "");
				groupName = groupName.replace(" ", "_");
				
				RuleGroup group = SemTranslator.toRuleGroupBeans( groupName, groupDescription);
